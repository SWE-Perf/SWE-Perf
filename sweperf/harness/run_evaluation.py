...
from sweperf.harness.constants import (
    APPLY_PATCH_FAIL,
    APPLY_PATCH_PASS,
...
from sweperf.harness.docker_utils import (
    remove_image,
    copy_to_container,
...
from sweperf.harness.docker_build import (
    BuildImageError,
    build_container,
    build_env_images,
    close_logger,
...
from sweperf.harness.grading import get_eval_report
from sweperf.harness.test_spec import make_test_spec, TestSpec, REPEAT_TIME
from sweperf.harness.utils import load_sweperf_dataset, str2bool
...
def get_dataset_from_preds(
    ...
    #     data["instance_id"] = f"{data['instance_id']}_{repeat}"
    #     dataset_.append(data)
    #     predictions[data["instance_id"]] = predictions[dataset[0][KEY_INSTANCE_ID]]
    # print(f"Using {len(dataset_)} instances for evaluation from {len(dataset)}.")
    # dataset = dataset_
    # prediction_ids = set(predictions.keys())
...
def make_run_report(
    ...
    func = partial(make_test_spec, is_eval=True)
    test_specs = list(map(func, full_dataset))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", default="SWE-Perf/SWE-Perf", type=str, help="Name of dataset or path to JSON file.")
